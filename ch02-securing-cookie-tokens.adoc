...
First a key pair is created. A private key is a huge secret random
number. The public key is calculated from the private key.
...
The encryption/decryption in the figure is the foundation for digital
signatures that will be used when John wants to make a payment. This
process is *not* suitable to send secret messages because the public
key is usually widely known.

The reverse process is also common. The public key is used to encrypt
and the private key is used to decrypt. This process is used to send
secret messages. It is not used in Bitcoin.
