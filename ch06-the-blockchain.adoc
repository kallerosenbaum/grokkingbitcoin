...
folder. Everyone has permission to read files from the shared folder,
but Lisa also has write permission. When Lisa writes the block to a
file in the shared folder, she _confirms_ the transactions in that
block.
...
This ordering of the steps is called _depth
first_, meaning that we always move downwards in the tree as far as we
can before moving sideways. But we will not go down in branches of the
tree that does not contain any interesting transactions.
...
order to verify that Tx~2~ is indeed included in the block. It starts
with verifying the partial merkle tree:

.The wallet verifies the partial merkle tree
image::{imagedir}/verify-partial-merkle-tree.svg[{big-width}]

The goal of this is to verify that the computed root of the partial
merkle tree matches the merkle root in the block header that the
wallet received from the full node. If they match, the full node has
proven that the txid of Tx~2~ actually exist in this block.

The process for computing the root is:
...
all 12 transactions, which would be about 3000 bytes.
...
| 100    | 8  | 25080    | 586
| 1000   | 11 | 250080   | 682
| 10000  | 15 | 2500080  | 810
| 100000 | 18 | 25000080 | 906
|===
...
following. The full node may have adopted a rule that allows double
the reward to Lisa. A typical full node operator would drop this block
as invalid, because that's not a rule he signed up for.
...
The last option is very useful if John is concerned that the full
