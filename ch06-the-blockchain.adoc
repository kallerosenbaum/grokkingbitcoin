...
bulletin board at the main entrance. The signature is required because only
...
. How many path elements do you need to construct a merkle proof for
...
and the bulletin board. What are the two risks of using a single key like
this?
...
. Recall the construction of the merkle proof in <<merkle-trees>>:
+
