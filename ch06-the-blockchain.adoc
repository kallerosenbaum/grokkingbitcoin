...
fully verify the blockchain to ensure that Lisa doesn't remove
...
instead of 7,200 cookie tokens as was the case in <<ch05>>. The idea is
that Lisa produces a new block every 10 minutes, which means that the
rewards of 7,200 CT per day are spread out over 144 blocks, since there
...
PKH~b~. Running this PKH through the hash functions gives the indexes
...
bits on those indexes are `1` then it will send the transaction. If
