...
.A blockchain is a chain of blocks. The blocks contains transactions and each block references its predecessor.
...
time is not exact, not even always increasing from block to
block.
...
if she tries to sheat. We will se how that works shortly.
...
<<wallet-connection>> of <<overview-wallets>>, where Bob's wallet was
notified of Alice's payment to Bob:
...
Just like Bob, suppose that John wants to receive notifications from a
full node only on transactions concerning his own wallet, that
contains two addresses, @~a~ and @~b~. He can make a network
connection to any of the full nodes at the office, for example the
cafe's. Then the wallet and the full node start talking:
...
to the full node what addresses belongs to the wallet.
...
block as the number of transactions grows.
...
transactions. For each new path element added, the size of the block
...
wallet receives a transaction and a mekle proof, it can check that the
transaction is contained in the block and that Lisa signed that
block. But it cannot verify

* the scriptSigs of the inputs. 
* that the spent outputs exists.
...
things on behalf of the wallet. This means that a full node can lie to
the wallet about all those things.
...
might be occations when the full node, deliberately or accidentally,
don't detect the error. Perhaps the cafe is in mascopy with Lisa to
fool John, who knows? The cafe and Lisa can, at least temporarily,
make John believe that he has received money that he didn't
receive. We will discuss this further in <<ch08>>.
...
* Connect to a _trusted node_. A trusted node is a full node that John
  ...
  traffic while stll being sure that he receives correct information
  from his full node.
...
Lisa can still censor transactions. She can refuse to include
transactions being sent to her into the blockchain. She has ultimate
power over what goes into the blockchain and what doesn't. In <<ch07>>
we will make it much harder for a single actor like Lisa to make such
decisions.
...
1. How does a block refer to the previous block?

2. What information does the merkle root commit to?
...
[start=3]
3. What information does Lisa's block signature commit to?

4. How is new cookie tokens (bitcoins) created?

5. What transactions would match a bloom filter containing only ones (`1`)?
...
[start=6]
6. What stuff from a transaction is tested by the full node when
determining whether to to send a transaction to the lightweight
wallet?

7. The hash functions used to create the bloom filter is not
_cryptographic_ hash function. Why not?

8. How many path elements do you need to construct a merkle proof for
a transaction in the above block?

==== Dig in

[start=9]
9. Draw the structure of a merkle tree of a block with 5 transactions.
+
image::{imagedir}/ex-merkle-tree-five-tx.svg[{half-width}]

10. Lisa signs all blocks with her block signing private key. The
public key is make public through several sources like the intranet
and the billboard. What are the two risks of using a single key like
this?

11. There are two places where a single person can censor transactions
or blocks. Which two places?

12. Suppose that Lisa replaces a block in the shared folder with a
...
[loweralpha]
 ...
 . has already downloaded the orignial block?
 
13. Make a bloom filter of eight bits of the two addresses @~1~ and
@~2~, where @~1~ hashes to the indices `6`, `1`, and `7` and @~2~
hashes to `1`, `5` and `7`. Then suppose that a full node tests the following transaction against the bloom filter:
+
image::{imagedir}/ex-bloom-filter-test-tx.svg[{half-width}]
+
Would the full node send this transaction to the lightweight wallet?

14. Recall the construction of the merkle proof in <<merkle-trees>>:
+
image::{imagedir}/ex-merkle-proof-two-tx.svg[{big-width}]

This merkle proof only proves one single transaction is in the
block. But we really wanted to prove two transactions, Tx~2~ and
Tx~3~. Please try to create such a proof, as small as possible. Assume
that the lightweight wallet already knows that the total number of
transactions in the block is 3, because in Bitcoin, it actually does.
...
* Lightweight wallets are used to save bandwith, but at the cost of
  reduced security.

* Lightweight wallets security is reduced because they cannot verify
  fully a transaction and because the full node can hide transactions
  from the wallet.
