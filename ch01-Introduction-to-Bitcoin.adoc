...
The Bitcoin network consists of thousands of computers that we call
nodes. Some of the nodes in the network are called Miners because they
"mine" new bitcoins with their computing power. Most nodes are
non-miners. They provide connectivity to the actors outside of the
network, like shoppers and exchanges.
...
securely so that only Alice can use them. Bob also has a key to mailbox
