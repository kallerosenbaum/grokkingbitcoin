...
OP_CHECKLOCK-TIMEVERIFY that implements the absolute time locked
output.
****
...
You may recall from <<transaction-fees>> that the transaction fee is
...
.A signature can commit to different parts of the transaction depending on the so called SIGHASH types. The grayed out parts are not included by the signature.
