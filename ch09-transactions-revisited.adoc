...
.No fee?
****
For the sake of simplicity, monst examples in this chapter don't pay
any fees.
****
...
The above example showed a usecase for absolute time locks on
...
the transaction is invalid until 1000 blocks have been mined since the
...
number of blocks. `03e8` is hex code for 1000.
...
which is illegal according to the scriptPubKey. The second one is at
...
ordinary, spendable, outputs. You cannot prove that yhey are
...
transaction output) set forever. This places an unneccesary burden on
...
This chapter has been a potpuurri of things you can do with
transactions.

Transactions and transaction outputs can be locked in different ways
to prevent spending of funds until a certain date or timespan has
occurred.
...
meaning for different values of the sequence numbers:
...
6. Suppose that Adam and Eve want to swap coins with eachother using
...
occurr. You and Ruth collaborate to create and broadcast a transaction
...
* Signatures can commit to different parts of the transaction using
  six combilnations of SIGHASH types.
