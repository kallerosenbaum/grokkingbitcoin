...
.Why "witness program"
****
It's called a witness program because it can actually be regarded as a
program of a weird language. In case of a p2wpkh output the witness
program is a single operator whose length defines its behaviour.
****
...
language. All segwit nodes will recognize verion `00`.
...
addition to the witness root hash, by makíng a merkle tree of all
transacion fees. Suggest how that merkle root could be commited to in
...
commitment in the previous excercise?
