...
operations. For example `OP_CHECKSEQUENCEVERIFY` (`OP_CSV`) and
`OP_CHECKLOCKTIMEVERIFY` (`OP_CLTV`) were introduced in the language
during 2015 and 2016. Let's have a look at how `OP_CLTV`, was
introduced.
...
like `OP_NOP2` when it succeeds. If `OP_CLTV` would be designed without
...
into numbers using a _base32_ lookup table. The first one of these
numbers is the witness version, `0`. The following numbers except the
last six numbers is the witness program. The last six numbers is the
checksum.
3. The checksum is verified, no errors were detected in this example.
4. The witness program is rewritten by writing each number as a 5
bit number.
5. Rearrange the bits in groups of 8 bits. Each such group represents
a byte of the witness program.
6. Amy extracts the witness program as
