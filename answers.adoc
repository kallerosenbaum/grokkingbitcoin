...
+
And F=2027-12-31 23:59:58
+
When we add the timestamp T~1~=2027-12-31 23:59:59 of B~1~ to
calculate MTP of B~2~, we must remove the block with lowest height
from the list of timestamps.
+
T~1~ is added somewhere to the right of F, because it's greater
than F. If any of a, b, c, d, e, or F is removed the median value will
move one step to the right, which is a value at most T~1~. In that
case the new MTP is < timeout.
+
If any of g, h, i, j or k is removed, we have added one value to the
right of F and removed one value to the right of F, in which case the
MTP remains F for B~2~. MTP for B~2~ is < timeout.
+
