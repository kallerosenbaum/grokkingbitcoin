...
5. 2 transactions on each blockchain. One for the contract transation
...
The second branch can be spent at earlies on new year's eve by:
...
The timelocked branch ensures that Beth has no power to collude with
...
5. The new behaviour of `OP_NOP5` must, if it succeeds, be exactly
like the old behaviour of `OP_NOP5`. This means that it should have no
...
nodes following different branches (forks) of the blockchain.
