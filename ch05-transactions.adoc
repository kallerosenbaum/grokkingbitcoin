...
****
Bitcoin addresses should not be reused. After spending an output, the
security of that output's address is reduced to the security of the
public key derivation function "only".
****
...
would also know that all John's outputs to PKH~1~ belongs to him. This
goes for change outputs too.

Luckily, the wallets will automate key creation for you, so you never
have to worry about key reuse.
...
in the cookie token every day paying 7200 new cookie tokens to
herself.
...
transactions. The transaction form a _transaction graph_. They are
...
have reached the four coinbase transactions. The coinbase transactions
must also be verified, so that there are exactly one coinbase per 24
hours and each coinbase creates exactly 7200 new cookie tokens. This
is what the UTXO set helps verifiers with. It keeps track of all
already verified unspent transaction outputs so that the verifiers
only have to follow the txids until it reaches an output that's in the
UTXO set.
...
* Money creation is verified by any coworkers, to make sure Lisa
