...
very difficult task. Each Bitcoin node is souvereign and no one
...
miner support. The _deployment machanisms_ have evolved over time and
...
transactions in the block. Remeber that a transaction first travels
...
.. Low bandwith mode, the `cmpctblock` is sent only upon request after
...
guaranteed not to get reorged by the Bitcoin Old branch. It's wipeout
protected by nature, but many changes are not wipeout protected by
nature.
...
An example of a change that's not wipeout protected by nature is an
...
would be vulnerable to wipeout. In order to protect against Bitcoin
New being wiped out in a reorg, they _added wipeout protection_ by
...
Soft fork:: The Old branch might get wiped out in a reorg. You can't
  protect the Old branch from wipeout because that would make this
...
Suppose that the above chain split has occured and you want to pay for
...
_probably_ (depending on the amount of "false" version 2 signalling)
...
a process for how to deploy several soft forks simultaneoulsy.
...
non-signalling miners a chance to upgrade. If they don't, their blocks
might be rejected.
...
the number of blocks that signalled support and then decided on a
