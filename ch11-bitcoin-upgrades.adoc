...
deployments, see <<block-version-number-deployments>>.
...
However, all deployments aren't as smooth.
...
hash rate would signal for segwit. There was also a group of people
...
A new BIP, BIP91, was written in May 2017 to satisfy both of the above
proposals. It would use BIP9 with a custom threshold. It would also
use a smaller span of blocks to gauge support:
...
uses a shorter timespan, 336 blocks instead of 2016 blocks, and it
...
segwit lock-in by dropping non-bit-1-signaling blocks.
...
and on 2017-07-20 the BIP got LOCKED_IN. It activated three days
later. Note that it was mainly miners who adopted BIP91. Normal users
typically used Bitcoin Core that didn't implement BIP91.

When miners activated BIP91, they started dropping blocks that didn't
signal bit 1. The result was that non-bit-1 blocks didn't make it into
the strongest chain, which quickly forced the remaining miners to
upgrade to segwit signaling blocks to avoid mining on a minority
chain.
...
rules. The rules are decided collectively by everyone, you and me, by
running our Bitcoin software of choice.
...
decide on a specific date or block
height that start dropping blocks





Some miners wanted segwit2x while others wanted just segwit. We
started seeing all combinations of signalling:

----
20000000: No signalling
20000002: BIP141
20000010: segwit2x
20000012: BIP141 and segwit2x.
----




A few BIPs were written to try to mitigate this issue, and the one
that finally got widespread adoption was BIP91.
...
. BIP141 Defined
. BIP141 Started
. BIP141 User adoption fast
. BIP141 Miner adoption slow

. 12 Mars 2017: Shaolin Fry proposes a flag-day activation. BIP148 is
born. Core didn't implement this BIP, but others did.

. BIP148 turned out to be controversial. Not everybody
liked it. Especially the segwit2x crowd opposed it because it
conflicts with their NYA using bit4. Exchanges and businesses
hesitant.

* BIP149 was born

. NYA born. No core devs supported it, but many prominent
orgs did. The support for 2MB was weak and would not be implemented in
Bitcoin Core. 

. Some people wanted to go through with BIP148 regardless if they're a
minority or not. They think risking a chain split if they only get a
mining minority would be worth it.

. BIP91 was born to avoid a BIP148 chain split and to avoid having
another deployment after BIP141 times out.

. Miners started adopting BIP91. 2017-07-20 BIP91 LOCKED_IN.

. This caused rest of miners to quickly start signal segwit.

. BIP91 ACTIVE on 477120, 2017-07-23. Miners started rejecting
non-segwit-signalling blocks. But by then all miners have upgraded to
BIP141 so little to no orphaned blocks occurred.

. BIP141 ACTIVE on 2017-08-09. 
...
timeout: 2017-11-15
LOCKED_IN: 2017-07-20 
ACTIVE: 2018-07-23 477102
Not active after LOCKED_IN
While active, drop non-segwit-signalling (bit 1) blocks

Strip:

Roles: Bitcoin Core, Users, Segwit2x, Miners, Hilliard

Core: Let's activate BIP141 using BIP9 bit 1!

Users: Yay!

Miners: No!

Users: Let's do BIP148 to reject non-bit-1-blocks on August 1.

Core: Too risky, that might cause a chain split. Will not implement.

Segwit2x: We want to activate segwit AND increase the maximum block
weight with a hard fork. We'll use BIP9 and bit 4 to activate this
change. We don't want JUST segwit.

Users: We'll implement BIP148 and go through with it.

Hilliard: Ok, How about BIP91? Segwit2x gets the segwit part of their
will, and the segwit crowd gets segwit.

Miners: Ok, fine. BIP91 ACTIVE on 2018-07-23

Users: Segwit is ACTIVE on 2017-08-09. Party!

Segwit2x: We'll go through with block size increase in mid-november
according to plan.

Core/Users: Hmm, I don't like the idea of a hard fork. Let's not do it.

Segwit2x: We'll go through with it anyway.

Segwit2x: Early november 2017: Ok, we'll not do it. Not enough user
support.
