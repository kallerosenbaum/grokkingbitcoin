...
full nodes change their software to drop all big blocks after a
...
eventually grow stronger than the big blocks brach. When that happens,
the big blocks branch will get wiped out and the remaining miners will
automatically switch to the small blocks branch, because the change is
a soft fork.
...
blocks that don't signal for the fork will be dropped. This will
effectively cause 100% compliance because non-compliant blocks don't
count anymore.
