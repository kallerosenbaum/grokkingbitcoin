...
=== Harden the security of the public key
...
Faiza's payment makes sure no typing errors happens by base58check decoding the address before signing the message.

.Faiza makes the payment to John
image::{imagedir}/payment-with-address.svg[{half-width}]
