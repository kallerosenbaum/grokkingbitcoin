...
still is. This means that if you can _calculate_ an input that gives a
certain PKH output, you still need to pre-image attack SHA256 (with
about 2^255^ guesses) in order to find the public key.
...
****
[start=8]
1. Imagine that John wants a cookie from the cafe. He has two
