...
online sales account. The cafe can then generate the xprv for the
online sales account using hardened extended private key derivation:
...
private key `m/1'` on the online sales web server. Using non-hardened
keys in the online sales account makes the cafe vulnerable to an
attacker stealing `m/1'/1` and `M/1'`. If that happens, all funds in
the online sales account will be stolen.
