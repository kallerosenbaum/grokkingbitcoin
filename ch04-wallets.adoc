...
back up a single random number, called a __seed__, once and
...
A password’s strength is measured in __entropy__. The higher the
entropy, the harder it is to guess the password. The word __entropy__,
...
a single random number called a __random seed__, the whole wallet +
could be backed up by writing down the seed on a piece of paper +
and storing it in a safe place (<<fig0407>>).
...
the root key of the tree (at the top) is called the __master private
key__. It’s the key from which all the rest of the keys are
derived. The master private key has two __child keys__: one that
...
Computer programmers often use the term _index_ to denote a
position in a list. It’s usually zero-based, meaning the first item in
the list has index 0, the second item has index 1, and so on. We’ll
use zero-based indexes throughout this book.
...
3. The descendant__ extended private keys__ of the master extended
...
HMAC __something__. A key is needed later, when you derive children of
the master xprv.
****
...
it’s called the _*master*_ private key because all other private keys
are derived from this single private key (and the chain code). The
right 256 bits become the __*chain code*__, used in the next step to
...
called a __mnemonic sentence__:
...
the checksum check will __probably __fail because she wrote the wrong
...
the result within 256-bit numbers, you use addition __modulo 2__^256^.
...
server, she can __calculate the online sales xprv__, as <<fig0421>>
shows.
...
catastrophic scenario? Yes, with __yet another key-derivation
function__! This new key-derivation function is called __hardened xprv
derivation__.
...
you hash the __private __key instead of the public key. An attacker
...
__public key multiplication__. Later subsections will show why child
xpub derivation, using __public key addition__, works, and how public
...
__elliptic curves__, and a solution is often referred to as a __point
on the curve__. You can now calculate a public key, which is a point
...
image::{imagedir}/u04-22.svg[{big-width}]
...
Exercises <<ex0407>> and <<ex0408>> assume that you read
