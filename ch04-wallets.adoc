...
hierarchical deterministic wallets, so that backups become dead
simple; You only need to backup a single random number, called a seed,
once and for all. We will take a really deep dive into how private and
public keys are created.
...
_elliptic curves_ and a solution is ofter referred to as a _point on
