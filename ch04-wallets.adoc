...
`depend`, the decoding would definately fail because there is no word
`depends` in the wordlist.
...
all words exist in the wordlist, would have a probability 1/16 to not
...
This resemblles xprv derivation. The difference is in what we do with
...
insecrue. Using non-hardened leaf keys in the on-line sales account
...
derive child public keys from parent public keys.
