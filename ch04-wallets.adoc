...
The cafe puts the xpub `M/1` on the the web server. By
convention, we use capital `M` to denote an xpub and
`m` to denote an xprv. `M/1` and `m/1` have the same
chain code, but `M/1` doesn't have the private key. Using the master
xpub you can create the whole xpub tree.
...
xpubs are generated differently than the extended
private keys. Please compare to the xprv derivation in
the margin.
...
The resemblance of xprv derivation is
...
explain it in simple terms, but if you think this is too much, you can skip the section on public key multiplication and jump to <<hardened-key-derivation>>.
...
calculate a child private key from a parent private key?
 ...
 "m/1/1" = "m/1" + "left half hash of index 1"
...
private keys are kept in the cafe's wallet and never touches the web
server.
