...
someone claiming to be someone else and paying money from that
someone's account. We solve that by introducing digital signatures to
the simple system.
...
list of transfers, there are three, involving Alice:
...
program called a cryptographic hash function. Suppose that you want to
...
image::{imagedir}/bits-bytes-hex.png[]
****

.Creating a cryptographic hash of a cat picture. Input is the cat picture and output is a big number of 32 bytes.
image::{imagedir}/hashing-a-cat.png[]

The hash in the picture is a 256 bit number. It means that to store
the number in a file, the file will be 32 bytes big, which is tiny
compared to the size of the 1.21 Megabytes cat picture.
...
picture from just the hash. Let's see what happens when you change the
cat picture just a tiny bit and run that cat picture through the
cryptographic hash function:

.hashing a modified cat picture. Can you spot the difference? The cryptographic hash function certainly did.
image::{imagedir}/hashing-a-modified-cat.png[]
...
the hash value? How does that work? The real answer is very complex,
so we will not go there. But to help you understand the operation of a
cryptographic hash function, we will create a very simplistic
cryptographic hash function. Well, it is not really cryptographic, we will come to that later. Let's just call it a hash function for now.
...
can construct a hash function using addition and wrap around to 0 when the result of an addition reaches 256 or above:
...
Cryptographic hash functions can be used to detect changes in
data. Suppose that you want to send your favorite cat picture to your
friend Fred via email, but you suspect that the picture may be
accidentaly corrupted during transfer. How would you and Fred make
sure that the picture Fred receives is exactly the same as the one you
send?
...
| image:{imagedir}/grumpy-cat-drawing.jpg[{thumb}] | dee6a5d375827436ee4b47a930160457901dce84ff0fac58bf79ab0edb479561 | nope
| My entire music collection | a5bcb2d99eb650d4756d200045fc50bf5529758c93dc0e75a5ac9c249c143f7a | nope
|===
...
****
image::{imagedir}/preimage-resistance.png[]
****
...
specific output. You have to repeatedly guess different inputs.
...
+
****
image::{imagedir}/simplistic-hash-algorithm-exercise.png[]
****

. Our simplistic hash function from secion <<cryptographic_hashing>> is not  a _cryptographic_ hash function. Which of the four properties of a cryptographic hash function is it lacking?
...
.A digital signature is the digital equivalent of a hand written signature.
image::{imagedir}/hand-written-vs-digital-signature.png[]
...
****
image::{imagedir}/2ndcol-encrypted-hash.png[]
****
