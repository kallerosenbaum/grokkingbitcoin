...
journey will be Bitcoin itself.
...
We have just witnessed the left side process of <<enc-dec-public-private,Figure 23>>:

.Ellen encrypts a message that only John can read.
image::{imagedir}/encrypt-decrypt.svg[{quart-width}]
...
The signing process use the right side of the diagram in <<enc-dec-public-private,Figure 23>>:
...
image::{imagedir}/sign-verify.svg[{quart-width}]
...
later use. The first tool we add is key pair generation.
...
attacker would then need both access to your hard-drive and your secret
