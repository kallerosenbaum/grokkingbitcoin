...
computers around the globe—the __Bitcoin network__, shown in
...
computers __Bitcoin nodes__, or just __nodes__. Any of the actors
...
pass the transaction to their neighbors, called __peers__.

3. Each computer updates its own copy of the __Bitcoin blockchain__, or the
__ledger__, with the new payment information.
...
.Transaction
****
A _transaction_ is a __payment__. The terms are interchangeable. We
...
secret number, called a __private key__, that only Alice has
...
the Bitcoin network. This is known as __relaying__. Alice’s
...
.Figure 1.5 Transactions arrive in different orders at different nodes. If all nodes wrote their transactions to the blockchain in order of arrival, the different nodes’ blockchains would differ.
[[txs-unordered]]
...
order Y, X!” This message, known as a __block__, is sent out on the
...
__Bitcoin wallet__. Several types of Bitcoin wallets are available for
...
Alice and Bob that the transaction went through, as figure
...
Extreme cases of inflation like this are called __hyperinflation__,
...
Moving value across national borders using national, or __fiat__,
...
nodes makes Bitcoin __decentralized__, as opposed to _centralized_
...
diminishing rate, according to a __predetermined__ schedule and will
...
image::{imagedir}/money-supply.svg[{big-width}]
...
alluring to them because of its price __volatility__, or tendency to
...
$1100 in a few weeks. This was clearly a so-called __bubble__, in which
...
any way. They’re tied to long strings of numbers called __public
keys__, explained in detail in <<ch02>>. The car manufacturer has made
...
A digital document has a __fingerprint__: a cryptographic hash that
...
Although all these risks are __possible__, most of them are unlikely.
...
__alt-coins__, meaning __alternative coins__. I’ll list a few alt-coins
...
.Network effect
[[network-effect]]
image::{imagedir}/network-effect.svg[{half-width}]
