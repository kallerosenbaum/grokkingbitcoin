...
.The Bitcoin network. Alice sends her transaction to a node in the network. The node will verify the transaction and forward it to other nodes. Eventually the transaction have reached all nodes in the network.
...
shortly have traveled the whole network while being verified by each
