...
the peer to peer network and within seconds reach all miners (well,
all full nodes actually). Do you remember this from <<ch01>>?

****
Transactions travel the network as we described in <<ch01>>.
...
transactions from reaching the whole network. A very pleasant effect
of using the peer to peer network for transactions is that the
...
communicate with it. All computers on the Internet have an IP
...
address. Lisas computer has the IP address 142.12.233.96 and runs a
...
it means that he trusts that John has paid enough transaction fee that
it will not be double spent
****
...
* the merkle root in the header can be reconstructed using the number
  of transactions (3) and the merkle proof.
...
probably have to buy one or two cookies more, but he will use Bitcoin
to do it.
...
* It consists of about 10000 publicly accessible full nodes. Add to
that several 10s of thousands full nodes that don't accept incoming
connections.
* Bitcoin's money supply is currently about 17000000 BTC.
* Each bitcoin is worth around $4400
* 250000 transaction per day
* An estimate of 150000 BTC, value $660M, being moved daily
* Total mining hashrate is about 8 Exahash/s
* Transaction fees paid each day totals around 150 BTC
...
The scenario in <<_the_network_protocol>> assumed that all nodes
...
office, the company's intranet, and asking colleagues. The same applies
...
must build their own UTXO set, so they still have to go through all
...
Which is about 8000 blocks back in the blockchain at release
...
and we have decided to ditch the cookie token project. Enjoy the last version.
...
| Transactions are broadcast to miners and others using the peer to peer network.
...
* New nodes synchronizes with the Bitcoin network to get up to date
