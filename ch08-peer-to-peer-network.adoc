...
`SHA256SUMS.asc`. She can verify that the program is in fact signed by
the private key corresponding to the public key in the signature
file. But how can she know that this key is not bogus? How does she
know that this key belongs to the Bitcoin Core team?
...
available at <<web-resource-install>>.
****
...
* have a fairly modern computer running a Linux operating system.
* have lots of available disk space. As of writing, about 150 GB is needed.
* have an Internet connection without a limited data plan.
* know how to start and use a command-line terminal.

If you don't have a Linux operating system, you can still use these
instructions, but you will have to install the version of Bitcoin Core
that's appropriate for your system. We'll help you select the correct
file to download a bit further down.
...
<<web-resource-download>>. As of writing the latest version of Bitcoin
...
(`linux-gnu`). When you are reading this, new version of Bitcoin Core
have probably been released. Consult <<web-resource-download>> to get
the right version of Bitcoin Core. Also, if you use another operating
...
We download the signature file `SHA256SUMS.asc` from the same server
as we downloaded the program from:
...
the correct signature file at <<web-resource-download>>.
...
The file we downloaded is named
`bitcoin-0.16.0-x86_64-linux-gnu.tar.gz` so we expect that the SHA256
...
Bitcoin Core signing key, we need the corresponding public key. As
noted in <<step-1>>, you should get the key from multiple sources and
verify that they all match before trusting that the key actually
belong to who you think it belongs to. It's also OK to download the
key from a single source and verify with other sources that the
_fingerprint_ of that key belongs to the Bitcoin Core team.

We will

* download the public key from `bitcoincore.org`, the official website
  of the Bitcoin Core team.
* consult the book Grokking Bitcoin to verify the fingerprint of that
  key
* verify the fingerprint with a friend

Let's start downloading the key as

----
$ wget https://bitcoincore.org/keys/laanwj-releases.asc
----

This downloads a public key into the file laanwj-releases.asc. The
owner of this key is "Wladimir J. van der Laan (Bitcoin Core binary
release signing key)", hence the name of the file. Let's examine this
file using a program called `gpg`, GNU Privacy Guard, that's installed
...
gpg says that the fingerprint is
`01EA5486DE18A882D4C2684590C8019E36C2E964` and that it belongs to
Wladimir J. van der Laan (Bitcoin Core binary release signing
key). But of course we don't just trust that. We want to make sure
beyond reasonable doubt that this is indeed the key we want.

We will now consult the book Grokking Bitcoin to check if the
fingerprint in that book matches the fingerprint of the
downloaded key. Let's have a look in <<step-1>> of that book. It says:
...
that. We will also call a friend we trust and have him/her read the
fingerprint to us.
...
You: Thank you, it matches my key, bye!

Donna: You're welcome, bye!

Donna's statement further stengthens your trust in this key. We think
we have collected enough evidence that this is in fact the correct key.

===== Install the public key as trusted on your computer

When we have verified that the key belongs to the Bitcoin Core team,
we will install that key onto our system and sign that key with a
private key that we own. We do that to remember this key as
trusted. The Bitcoin Core team will probably release new versions of
Bitcoin Core in the future. If pgp remembers this public key as
trusted, you don't have to go through all these key verification steps
again when you upgrade.

The process is:

1. Import the Bitcoin Core public key into gpg
2. Create a key of your own
3. Sign the Bitcoin Core public key with your own private key
...
The above command imports the key we downloaded and adds it to your
list of known keys. But the output of the above command, mentions that
`no ultimately trusted keys found`. This means that this key is not
signed by any key that you trust. You have only imported the key. In
gpg, keys can sign other keys to certify that the signed key is
legit.

You will now sign the key you just verified, but for that you need to
have a key of your own to sign with. Gpg let's you create a key with
...
4. Imported the public key into GnuPG
5. Signed that key with our own private key to tell GnuPG that the
  Bitcoin Core key is legit.
6. Verified the signature of the `SHA256SUMS.asc` file.
...
We can use the `bitcoin-cli` program to query the running node about
the progress:
...
Keep this daemon running. We will get back to this daemon in an
appendix to give you a concrete guide on how to use `bitcoin-cli` to
